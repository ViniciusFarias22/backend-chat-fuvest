...
if not GROQ_API_KEY:
    print("⚠️ AVISO: GROQ_API_KEY não encontrada nas variáveis de ambiente do Render")
...
4. Dar DICAS de estudo e técnicas de aprendizado

Matérias: Matemática, Português, Física, Química, Biologia, História, Geografia, Redação, etc.

Seja sempre paciente, motivador e responda em português do Brasil."""

@app.get("/")
async def root():
    return {"status": "online", "message": "FUVEST Prep Chat API - Groq"}
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code != 200:
                ...
                raise HTTPException(status_code=response.status_code, detail=f"Erro na API Groq: {error_msg}")
